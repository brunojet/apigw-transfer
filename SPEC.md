...
| Fallback com lock **não-bloqueante** (202 + `Retry-After`) — assíncrono, `202` a todos inclusive a quem pegou o lock (ADR 0001); na PoC via autoinvocação assíncrona da Lambda, no desenho final pelo BFF | ✅ Escolhida — evita Lambda ocioso esperando outra invocação terminar (custo) e evita que erros de IAM/permissão (`AccessDenied`) sejam mascarados como "concorrência normal"; qualquer erro que não seja literalmente "lock já existe" vira erro real, não retry silencioso |
